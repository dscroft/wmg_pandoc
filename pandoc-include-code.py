...
def truncate( contents, t=0 ):
    return [ i if len(i) < t else f"{i[:t-3]}..." for i in contents ]
...
def process(key, value, format, meta):
    if key == "CodeBlock":
        ...
        try:
            mn = min( ( int(i[-1]) for i in keyvals if i[0] in ("start", "startLine", "begin", "beginLine") ) ) -1
        except ValueError: 
            mn = 0

        try:
            mx = min( ( int(i[-1]) for i in keyvals if i[0] in ("stop", "stopLine", "end", "endLine" ) ) )
        except ValueError:
            mx = len( contents )
        ...
        # code needs to be truncated
        if trunc: contents = truncate( contents, trunc )
